...
void setaddr(char str[]){
	char trash[10];
	char trash2;
	int check = sscanf(str,"%s %x %c",trash,&Progaddr,&trash2);
	if(check!=2){
		printf("Invalid Input.\n");
		return;
	}
...
int convert_to_command(char *str,char *strcopy){ // 문자열이 어떤 명령어를 포함하는지 검사하는 함수이다
	...
	else if(!strncmp("progaddr",str,8))
		return 18;
	else if(!strncmp("loader",str,6))
		return 19;
	else if(!strcmp("bp",str)){
		LInsert(&History,strcopy); return 20;}
	else if(!strcmp("bp clear",str)){
		LInsert(&History,strcopy); return 21;}
	else if(!strncmp("bp ",str,3))
		return 22;
	else
		return -1;
